...
class TrasactionReportController extends Controller
{
	...
	public function index(Request $request){
		...
		$data['detail_expance'] = TrxExpenceDetailModel::select('*',DB::raw('SUM(trx_expance_details.total) AS total_details_bysub group by sub_category_id'))->with(['subcategory','master'])
						->whereHas('master', function($q) {
							$q->where('user_id', '=', Session::get('Users.id'));
						})
						->groupby('sub_category_id');
